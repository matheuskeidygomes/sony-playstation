...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between'>
        ...
        <div>
            <img className='w-20 h-20 animate-pulse1 ml-8 mt-4' src={XboxLogo} alt="xbox-logo" />
        </div>
        
        <div className='flex items-center justify-between md:grow'>
            
            <nav className={`${toggleSidebar ? "block" : "hidden"} md:block`}>
                <ul className={`flex flex-col z-50 md:flex-row md:min-h-fit md:static ${toggleSidebar && 'items-center min-h-full w-full p-5 fixed top-0 right-0 animate-slide bg-[#ffffff1d] backdrop-blur-md md:bg-transparent'}`}>
